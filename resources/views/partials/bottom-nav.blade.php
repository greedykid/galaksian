<!-- ========================================================= -->
        ...
        <!-- ========================================================= -->
        <nav class="fixed bottom-0 inset-x-0 mx-auto z-40 w-full max-w-[430px] bg-white border-t border-zinc-200 px-3 py-2 flex items-center justify-around shadow-[0_-4px_20px_rgba(0,0,0,0.03)]">
            <!-- 1. Home / Beranda -->
            <button 
                @click="goToTab('home')" 
                :class="activeTab === 'home' && !activeSubView ? 'text-zinc-950 font-bold' : 'text-zinc-400 hover:text-zinc-600 font-medium'"
            ...
            <button 
                @click="goToTab('cart')" 
                :class="activeTab === 'cart' && !activeSubView ? 'text-zinc-950 font-bold' : 'text-zinc-400 hover:text-zinc-600 font-medium'"
                ...
                <div class="relative">
                    ...
                    <template x-if="cart.total_qty > 0">
                        <span class="absolute -top-1.5 -right-2.5 bg-[#E60012] text-white text-[9px] font-bold px-1.5 py-0.2 rounded-full tabular" x-text="cart.total_qty"></span>
                    </template>
                </div>
            ...
            <button 
                ...
                :class="(activeTab === 'transactions' || activeSubView === 'order-detail' || activeSubView === 'qris-payment') ? 'text-[#E60012] font-bold' : 'text-zinc-400 hover:text-zinc-600 font-medium'"
                class="flex flex-col items-center gap-1 min-w-[56px] min-h-[44px] justify-center transition">
                <svg class="w-5 h-5 stroke-current fill-none" viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <button 
                ...
                :class="activeTab === 'profile' && !activeSubView ? 'text-zinc-950 font-bold' : 'text-zinc-400 hover:text-zinc-600 font-medium'"
                class="flex flex-col items-center gap-1 min-w-[56px] min-h-[44px] justify-center transition">
                <svg class="w-5 h-5 stroke-current fill-none" viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
