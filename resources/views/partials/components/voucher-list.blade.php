...
<template x-for="v in (voucherList || []).slice(0, showAllVouchers ? (voucherList || []).length : 2)" :key="'voucher-' + v.code">
    <div class="{{ $cardClass }} {{ $isCard ? '' : 'flex items-center justify-between mb-2' }}"
         :class="v.type === 'fixed' ? 'border-blue-200 bg-blue-50/20' : 'border-amber-400 bg-amber-50/15'">

        {{-- Left: icon + title/desc --}}
        <div class="{{ $isCard ? '' : 'flex items-center gap-2.5' }}">
            <div class="w-9 h-9 rounded-xl shrink-0 flex items-center justify-center"
                 :class="v.type === 'fixed' ? 'bg-blue-100 text-[#1657FF]' : 'bg-pink-100 text-pink-500'">
        ...
        @if ($isCard)
            <div class="border-t border-dashed pt-2 flex items-center justify-between"
                 :class="v.type === 'fixed' ? 'border-blue-200' : 'border-amber-300'">
                <div class="flex items-center gap-1.5 text-[10px] font-semibold"
                     :class="v.type === 'fixed' ? 'text-zinc-400' : 'text-emerald-700'">
                    <span class="w-1.5 h-1.5 rounded-full"
                          :class="v.type === 'fixed' ? 'bg-zinc-400' : 'bg-[#00D06C]'"></span>
                    <span x-text="getVoucherState(v.code) === 'applied' ? t('voucher_active_applied', 'Voucher aktif terpasang') : (getVoucherState(v.code) === 'claimed' ? t('ready_to_use_title', 'Siap digunakan') : t('available', 'Tersedia'))"></span>
                </div>
                <button
                    @click="handleVoucherAction(v.code)"
                    :class="getVoucherState(v.code) === 'applied' ? 'bg-[#00D06C] text-white hover:bg-emerald-600' : (getVoucherState(v.code) === 'claimed' ? 'bg-amber-400 text-zinc-950 hover:bg-amber-500' : 'bg-[#1657FF] text-white hover:bg-blue-700')"
                    class="px-4 py-1.5 font-extrabold text-xs rounded-lg shadow-2xs transition active:scale-95 cursor-pointer">
                    <span x-text="getVoucherState(v.code) === 'applied' ? t('used_btn', 'Dipakai') : (getVoucherState(v.code) === 'claimed' ? t('use_btn', 'Pakai') : t('claim_btn', 'Klaim'))"></span>
                </button>
            </div>
        @else
            <button
