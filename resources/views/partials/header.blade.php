<!-- ================= TOP APP BAR (FIXED ROCK-SOLID) ================= -->
        <header 
            class="fixed top-0 inset-x-0 mx-auto z-30 w-full max-w-[430px] transition-colors duration-150"
            :class="(activeTab === 'home' && !activeSubView) ? 'bg-[#1657FF] text-white border-b border-[#1657FF]' : 'bg-white/95 backdrop-blur-md border-b border-zinc-200 shadow-2xs text-zinc-900'">
            <div class="px-4 py-2.5 flex items-center justify-between">
                <!-- Brand Identity -->
                <button @click="goToTab('home')" class="flex items-center gap-2 text-left group">
                    <div 
                        class="w-7 h-7 rounded-lg flex items-center justify-center font-black text-xs tracking-wider transition shadow-xs"
                        :class="(activeTab === 'home' && !activeSubView) ? 'bg-[#00D06C] text-white' : 'bg-zinc-950 text-white group-hover:bg-[#1657FF]'">
                        G
                    </div>
                    <div>
                        <div class="flex items-center gap-1.5">
                            <span class="font-extrabold text-[15px] tracking-tight" :class="(activeTab === 'home' && !activeSubView) ? 'text-white' : 'text-zinc-950'">GALAKSIAN</span>
                ...
                <div class="flex items-center gap-1.5">
                    ...
                    <a :href="waCsUrl" target="_blank" :title="t('contact_wa_cs_title', 'Hubungi WhatsApp CS')" 
                        ...
                        :class="(activeTab === 'home' && !activeSubView) ? 'text-white border border-white/20 bg-white/10 hover:bg-white/20' : 'border border-zinc-200 text-zinc-700 hover:text-emerald-700 hover:border-emerald-300 hover:bg-emerald-50'">
                    ...
                    <div class="relative" @click.away="langDropdownOpen = false">
                        <button 
                            ...
                            type="button"
                            class="h-8 px-2 rounded-lg flex items-center gap-1 transition select-none min-h-[32px]"
                            :class="(activeTab === 'home' && !activeSubView) ? 'bg-white/10 hover:bg-white/20 border border-white/20 text-white' : 'border border-zinc-200 bg-white hover:bg-zinc-50 text-zinc-800 shadow-2xs'"
                            :title="currentLang === 'id' ? 'Ubah Bahasa Aplikasi' : 'Change App Language'"
                            aria-label="Language selector">
                            <span class="text-[11px] font-bold tracking-tight" :class="(activeTab === 'home' && !activeSubView) ? 'text-white' : 'text-zinc-900'" x-text="currentLang.toUpperCase()"></span>
                            <svg class="w-3 h-3 transition-transform duration-150" :class="[langDropdownOpen ? 'rotate-180' : '', (activeTab === 'home' && !activeSubView) ? 'text-white/80' : 'text-zinc-400']" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                    ...
                    <button @click="showNotifications = !showNotifications" 
                        class="w-8 h-8 rounded-lg flex items-center justify-center transition relative"
                        :class="(activeTab === 'home' && !activeSubView) ? 'text-white border border-white/20 bg-white/10 hover:bg-white/20' : 'border border-zinc-200 text-zinc-700 hover:bg-zinc-50'">
                        <svg class="w-4 h-4 stroke-current fill-none" viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
