...
<div x-show="activeTab === 'cart' && !activeSubView" class="space-y-3 bg-white min-h-screen">
    ...
    <!-- 1. Dedicated Top Header (Matching media_1788725780938.png) -->
    <div class="sticky top-0 z-30 bg-[#1657FF] text-white px-4 py-3 flex items-center justify-between shadow-xs -mx-px w-[calc(100%+2px)]">
        <button @click="goToTab('home')" class="p-1 -ml-1 text-white hover:text-white/80 transition flex items-center gap-2">
            ...
            <span class="text-base font-bold text-white tracking-tight" x-text="t('cart_title', 'Keranjang')">Keranjang</span>
        </button>
    ...
    <div class="bg-white border-b border-zinc-100 px-4 py-3 flex items-center justify-between gap-3">
        <div class="flex items-center gap-3 min-w-0">
            ...
            <div class="min-w-0">
                ...
                <p class="font-bold text-xs text-zinc-900 line-clamp-1 mt-0.5" x-text="defaultAddress ? defaultAddress.address : 'Belum ada alamat tersimpan'"></p>
                <p class="text-[11px] text-zinc-500 line-clamp-1" x-text="defaultAddress ? ((defaultAddress.city || '') + (defaultAddress.postal_code ? ' ' + defaultAddress.postal_code : '') + (defaultAddress.province ? ', ' + defaultAddress.province : '')) : 'Silakan tentukan alamat tujuan'"></p>
            </div>
        </div>
        <button @click="openAddressModal()" class="px-3.5 py-1 rounded-lg border border-blue-200 text-[#1657FF] hover:bg-blue-50 text-xs font-semibold transition shrink-0" x-text="t('edit_btn', 'Ubah')">
            Ubah
        </button>
    </div>

    <!-- 3. Kirim Sebagai Bingkisan Card Banner -->
    <div class="mx-4 my-1 p-3.5 rounded-2xl border border-blue-200 bg-blue-50/40 flex items-center justify-between shadow-2xs">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-[11px] text-zinc-500" x-text="t('gift_desc', 'Tambahkan kartu ucapan & packing khusus')">Tambahkan kartu ucapan & packing khusus</p>
            </div>
        </div>
        <button 
            ...
            class="w-7 h-7 rounded-full flex items-center justify-center shrink-0 transition shadow-2xs"
            :title="giftOptionEnabled ? 'Bingkisan Aktif' : 'Aktifkan Bingkisan'">
                ...
                <div class="mx-4 py-3.5 border-b border-zinc-100 space-y-2.5">
                    ...
                    <div x-show="editingNoteItemId !== item.id">
                        <button 
                            ...
                            <span x-text="itemNotes[item.id] ? ('Catatan: ' + itemNotes[item.id]) : t('add_note_btn', 'Tambah Catatan')">Tambah Catatan</span>
                        </button>
                    </div>
                    ...
                    <div x-show="editingNoteItemId === item.id" class="p-2.5 bg-zinc-50 border border-zinc-200 rounded-xl space-y-2" x-cloak>
                        <textarea 
                            ...
                            rows="2" 
                            placeholder="Tulis catatan varian, kemasan, atau instruksi jastip..."
                        ...
                        <div class="flex justify-end gap-2 text-xs">
                            <button @click="cancelItemNote()" class="px-3 py-1 rounded-lg text-zinc-600 hover:bg-zinc-200 font-medium">Batal</button>
                            <button @click="saveItemNote(item.id)" class="px-3.5 py-1 bg-[#1657FF] text-white font-bold rounded-lg hover:bg-blue-700">Simpan</button>
                        </div>
                    </div>
                </div>
            </template>
        ...
        <div class="pt-2">
            ...
            <div class="mx-4 rounded-2xl border-2 border-amber-400 bg-amber-50/15 p-3.5 space-y-2.5 shadow-2xs">
                ...
                <div class="border-t border-dashed border-amber-300 pt-2 flex items-center justify-between">
                    ...
                    <button 
                        @click="applyPromoCode('GALAKSIAN10')" 
                        class="px-4 py-1.5 bg-amber-400 hover:bg-amber-500 text-zinc-950 font-extrabold text-xs rounded-lg shadow-2xs transition active:scale-95">
                        Dipakai
                    </button>
                </div>
            </div>
            ...
            <div class="mx-4 mt-2.5 rounded-2xl border border-blue-200 bg-blue-50/20 p-3.5 space-y-2.5 shadow-2xs">
                ...
                <div class="border-t border-dashed border-blue-200 pt-2 flex items-center justify-between">
                    ...
                    <button 
                        @click="applyPromoCode('POTONGAN15K')" 
                        class="px-4 py-1.5 bg-[#1657FF] hover:bg-blue-700 text-white font-bold text-xs rounded-lg shadow-2xs transition active:scale-95">
                        Klaim
                    </button>
                </div>
            </div>
